...
def _overpass_query(query, cache_name):
    ...
    # 离线模式: 缓存缺失时直接返回空, 绝不发起联网请求
    if getattr(cfg, "OFFLINE", False) and not cache_path.exists():
        return {"elements": []}
...
def fetch_osm_faults(bbox):
    """下载地质断裂带(用于 dist_fault 距离波段, 对应特征波段 17)。"""
    bbox_str = f"{bbox[1]},{bbox[0]},{bbox[3]},{bbox[2]}"
    query = f"""
    [out:json][timeout:180];
    (
      way["natural"="fault"]({bbox_str});
      way["geological"="fault"]({bbox_str});
      way["fault"="yes"]({bbox_str});
    );
    out geom;
    """
...
def fetch_osm_vegetation(bbox):
    """下载植被/林地(用于 vegetation_height 波段, 对应特征波段 20)。"""
    bbox_str = f"{bbox[1]},{bbox[0]},{bbox[3]},{bbox[2]}"
    query = f"""
    [out:json][timeout:180];
    (
...
def build_seismic_risk(dem, transform, crs):
    ...
    dy, dx = np.gradient(dem.astype(np.float32), cellsize_m, cellsize_m)
