...
## V3 三算法对比框架 (v3.20260710)

## V3 三算法对比框架 (v3.20260710)

在 V2 基础上新增纯群体智能对比框架（RF 成本表面）：

| 算法 | 范式 | 核心机制 |
|------|------|---------|
| **A\*** | 图搜索（确定性最优） | 8邻域 + Octile启发式 + 硬约束掩膜 |
| **IPSO-SA** | 群体智能（对照组） | PSO惯性递减(0.9→0.4) + SA退火跳出局部最优 |
| **DBO** | 群体智能（创新算法,2022） | 蜣螂五行为: 滚球/跳舞/育雏/觅食/偷窃 |

### V3 对比结果 (RF成本表面, 10条线路)

| 指标 | A\* | IPSO-SA | DBO |
|------|-----|---------|-----|
| Hausdorff 均值 | 6,227m | 6,132m | 6,132m |
| 重叠率均值 | 21.7% | 23.3% | 23.3% |
| 质量门控通过 | **10/10** | 8/10 | 8/10 |

---

## V4 CNN增强版 (v4.20260718)

V4 在 V3 基础上新增 AI 增强：U-Net 像素级成本预测 (7.8M参数, RTX 4060 GPU训练)。

### V4 对比结果 (CNN成本表面, 10条线路)

| 指标 | A\* | IPSO-SA | DBO |
|------|-----|---------|-----|
| Hausdorff 均值 | 5,656m | **4,667m** | 12,526m |
| 重叠率均值 | 35.6% | 30.9% | 19.3% |
| 质量门控通过 | 9/10 | 7/10 | 1/10 |
| vs RF (V3) | ↓9% | **↓24%** | — |

统计检验: 配对t检验, n=10, Cohen's d=0.22. IPSO-SA在CNN上最好.

### V4 AI 模块

| 模块 | 功能 | 状态 |
|------|------|------|
| `cnn_model/train_cnn.py` | U-Net 成本表面训练 | ✅ |
| `cnn_model/infer.py` | 全图推理 (18s GPU) | ✅ |
| `cnn_model/train_heuristic.py` | MLP 注意力增强A*启发式 | ⚠️ 训练完成, 待集成 |
...
└── src/
    ├── base_planner.py       # 群体智能基类 (策略模式)
    ├── ipso_sa_planner.py    # IPSO-SA 规划器
    ├── dbo_planner.py        # DBO 蜣螂优化规划器
    └── output_v3.py          # 统一输出 (SHP/GeoJSON/收敛图/统计)
```
