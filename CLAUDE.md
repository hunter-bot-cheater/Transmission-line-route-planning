...
```
Phase 1: Data Acquisition    archive/历史版本/shared/data_acquisition.py
          ↓                  DEM (SRTM) + OSM (Overpass API) + power-line shapefile + risk proxies
Phase 2: Preprocessing       archive/历史版本/v2_20260525/src/preprocessing.py
          ↓                  Terrain factors (7 layers) → aligned 90m grid → hard/soft constraint masks
Phase 3: Cost Modeling       archive/历史版本/v2_20260525/src/cost_model.py
          ↓                  18-dim feature stack → pseudo-label generation → Random Forest (R² > 0.97)
Phase 4: Path Planning       archive/历史版本/v2_20260525/src/path_planning.py
          ↓                  Cost fusion → A* search (8-neighbor, octile heuristic) → smoothing → 7 quality gates
Phase 5: Output              archive/历史版本/v2_20260525/src/output.py
...
v2 is v1 with tightened parameters and quality gates added. The pipeline structure is identical. `archive/历史版本/shared/data_acquisition.py` is used by both. v1 source (`archive/历史版本/v1_20260525/src/`) contains older copies of preprocessing, cost_model, path_planning, and output — when fixing a bug that exists in both versions, apply the fix to v2 first, then backport to v1 only if needed. Duplicated utility functions (`geo_to_grid`, `compute_path_length_km`, RDP fallback, etc.) should eventually be extracted to `archive/历史版本/shared/path_utils.py`.
...
From `docs/需求设计/CODING_STANDARDS_v2.md`:
...
`archive/历史版本/shared/data_acquisition.py` 中的 `_resample_dem_to_standard` 在管线开始前将 DEM 重采样到 90m，导致所有地形因子在粗网格上计算，微地形被抹平。改用 30m SRTM DEM 原生分辨率计算地形后再降采样，V2 Hausdorff 从 10954m 改善至 6419m，质量通过率从 1/10 提升至 10/10。注意：archive/历史版本/shared/data_acquisition.py 不应包含预重采样逻辑，V3 如需适配高分辨率 DEM 应使用自己的 `data_acquisition_v3.py`。

## 版本说明

| 版本 | 目录 | 内容 |
|------|------|------|
| V1 | `archive/历史版本/v1_20260525/` | 宽松约束原型 |
| V2 | `archive/历史版本/v2_20260525/` | A\* + 7项质量门控 (当前稳定版) |
| V3 | `archive/历史版本/v3_20260710/` | 纯群体智能框架 (A\*/IPSO-SA/DBO + RF成本表面) |
| V4 | `archive/历史版本/v4_20260718/` | CNN增强版 (U-Net成本预测 + MLP启发式 + 完整AI管线) |
...
```
docs/测试运维/变更记录/<YYYY-MM-DD>_项目修改记录.txt
```
