...
### 成本表面过于平滑导致三算法均收敛到接近直线

v3 运行发现 A\*、IPSO-SA、DBO 三种算法在 10 条线路上均生成接近直线的路径。根因分析：
- `fuse_cost_surface` 中的走廊偏好（`corridor_bonus`）在起止点连线 1km 内提供 2%-98% 成本折扣，形成"低成本高速公路"
- 当前硬约束仅屏蔽极端情况（坡度>45°、高程>3000m），中等障碍（30°-40° 坡度、小河沟）在 90m 栅格下不可见
- 12.5m DEM 重采样到 90m 后微地形被抹平
- 伪标签中 `dist_existing` 权重 0.65 进一步强化直线偏好

后果：三个算法 Haussdorff 距离偏大（均值 6000-11000m），重叠率偏低（12%-23%）。需调整成本表面参数（削弱 corridor_bonus、降低 dist_existing 衰减距离、收紧中等地形约束）来恢复路径的地形适应性。
