"""
v2_strict: 模块2 — 数据预处理(严格化版本)
版本: v2.20260525
作者: path_planning_team
变更记录:
  - v2.20260525: 收紧硬约束(坡度28°/水域100m/高程3000m/保护区200m缓冲/建筑密度)
  - v1.20260525: 初始版本(宽松阈值)
依赖: shared/data_acquisition, v2/config
"""
import sys
from pathlib import Path
# v2_strict: 使用shared中的通用模块
...
# ============================================================
# 约束掩膜生成 — v2_strict: 收紧所有阈值
# ============================================================
def generate_hard_mask(data, transform, shape):
    """
    v2_strict: 生成硬约束掩膜 (0=禁止建设, 1=允许)
    禁止区域:
      1. 坡度 > 28° (v1=35°)
      2. 水域 + 100m缓冲区 (v1=50m)
      3. 自然保护区 + 200m缓冲区 (v1=无缓冲)
      4. 高程 > 3000m (v1=3500m)
      5. 密集建筑区 > 500栋/km² (v2新增)
    """
    print("[Phase2] 生成硬约束掩膜 (v2_strict)...")
    hard_mask = np.ones(shape, dtype=np.uint8)
    pixel_deg = abs(transform.a)

    # v2_strict: 1. 坡度约束 — 从35°收紧至28°
    slope = data.get("slope")
    if slope is not None:
        slope_aligned = resample_to_target(slope, transform, transform, shape)
        slope_blocked = slope_aligned > cfg.MAX_SLOPE
        hard_mask[slope_blocked] = 0
        print(f"  坡度>{cfg.MAX_SLOPE}°禁止: {slope_blocked.sum()} 像元")
    ...
    if water_gdf is not None and len(water_gdf) > 0:
        ...
        buffer_pixels = int(buffer_deg / pixel_deg)
        if buffer_pixels > 0:
            water_buffer = binary_dilation(water_raster > 0, iterations=buffer_pixels)
            hard_mask[water_buffer] = 0
            print(f"  水域+{cfg.WATER_BUFFER}m缓冲区禁止: {water_buffer.sum()} 像元")
        elif cfg.WATER_BUFFER > 0:
            # buffer=0时仅阻断水域本身
            hard_mask[water_raster > 0] = 0
            print(f"  水域禁止(无缓冲): {(water_raster > 0).sum()} 像元")
        else:
            print(f"  水域未纳入硬约束(WATER_BUFFER=0)")
    else:
        print("  水域数据缺失, 跳过水域约束")

    # v2_strict: 3. 自然保护区 + 200m缓冲区 (v1无缓冲)
    ...
    if protected_gdf is not None and len(protected_gdf) > 0:
        ...
        if cfg.PROTECTED_BUFFER > 0:
            buffer_deg = cfg.meters_to_deg(cfg.PROTECTED_BUFFER)
            ...
            print(f"  自然保护区+{cfg.PROTECTED_BUFFER}m缓冲区禁止: {protected_buffer.sum()} 像元")
        else:
    ...
    # v2_strict: 4. 高海拔 — 从3500m降低至3000m
    dem_aligned = data.get("dem")
    if dem_aligned is not None:
        dem_r = resample_to_target(dem_aligned, transform, transform, shape)
        elevation_blocked = dem_r > cfg.MAX_ELEVATION
        hard_mask[elevation_blocked] = 0
        print(f"  高程>{cfg.MAX_ELEVATION}m禁止: {elevation_blocked.sum()} 像元")

    # v2_strict: 5. 密集建筑区 — >500栋/km²禁止穿越
    ...
    if build_density is not None and np.any(build_density > 0):
        ...
        urban_blocked = bd_aligned > cfg.BUILDING_DENSITY_LIMIT
        hard_mask[urban_blocked] = 0
        print(f"  建筑密度>{cfg.BUILDING_DENSITY_LIMIT}栋/km2禁止: {urban_blocked.sum()} 像元")
    else:
        print("  建筑密度数据缺失, 跳过建筑约束")
...
def generate_soft_mask(data, transform, shape):
    """
    v2_strict: 生成软约束掩膜 (0-1, 值越高成本越高)
    """
    print("[Phase2] 生成软约束掩膜...")
    soft_mask = np.ones(shape, dtype=np.float32)

    # v2_strict: 优先使用预计算的土地利用栅格 (来自align_all_rasters)
